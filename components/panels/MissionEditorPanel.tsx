...
const MissionEditorPanel: React.FC = () => {
    ...
    const validationErrors = useMemo(() => {
        ...
        if (waypoints.some(wp => wp.alt < 20 && wp.altType === 'MSL')) errors.push("At least one waypoint is below the 20m minimum safe altitude.");
        const positions = new Set(waypoints.map(wp => `${wp.lat.toFixed(6)},${wp.lng.toFixed(6)}`));
        if (positions.size !== waypoints.length) errors.push("Duplicate waypoints found at the same coordinates.");
        let estimatedTime = 0;
        for (let i = 0; i < waypoints.length - 1; i++) {
            const from = waypoints[i];
            const to = waypoints[i+1];
            estimatedTime += getDistance(from, to) / from.speed;
        }
        if (estimatedTime > 20 * 60) errors.push(`Est. flight time (${Math.round(estimatedTime / 60)} min) may exceed standard battery life.`);
        return errors;
    ...
    const handleUpdate = (id: string, field: keyof Waypoint, value: string) => {
        ...
        if(!isNaN(numValue)) {
            ...
            if (waypointToUpdate) dispatch({ type: 'UPDATE_WAYPOINT', payload: { ...waypointToUpdate, [field]: numValue } });
        }
    };
        ...
        <div className="space-y-4">
            {validationErrors.length > 0 && (
                <div className="bg-orange-500/10 border border-orange-500/30 p-3 rounded-lg space-y-2">
                    <div className="flex items-center space-x-2"><AlertTriangle /><h4 className="text-md font-bold text-orange-400">Plan Validation</h4></div>
                    <ul className="list-disc list-inside text-sm text-orange-300 space-y-1">{validationErrors.map((error, i) => <li key={i}>{error}</li>)}</ul>
                </div>
            )}
            <div>
                <h3 className="text-md font-bold text-slate-100 mb-1">Mission Details</h3>
                <div className="space-y-3 bg-base-100 p-3 rounded-md">
                    <input type="text" value={currentMission.name} onChange={(e) => dispatch({ type: 'SET_MISSION_NAME', payload: e.target.value })} className="w-full bg-base-300 p-2 rounded-md focus:ring-1 focus:ring-primary outline-none" />
                    <div className="grid grid-cols-2 gap-2">
                        <input type="number" placeholder="Home Lat" value={currentMission.homePosition?.lat ?? ''} onChange={e => dispatch({type: 'SET_HOME_POSITION', payload: { ...currentMission.homePosition!, lat: parseFloat(e.target.value) || 0 }})} className="w-full bg-base-300 p-2 rounded-md focus:ring-1 focus:ring-primary outline-none" />
                        <input type="number" placeholder="Home Lng" value={currentMission.homePosition?.lng ?? ''} onChange={e => dispatch({type: 'SET_HOME_POSITION', payload: { ...currentMission.homePosition!, lng: parseFloat(e.target.value) || 0 }})} className="w-full bg-base-300 p-2 rounded-md focus:ring-1 focus:ring-primary outline-none" />
                    </div>
                </div>
            </div>
            ...
            <div>
                ...
                <div className="flex space-x-2">
                    <button onClick={() => setMode(mode === 'BOUNDARY' ? 'WAYPOINT' : 'BOUNDARY')} className={`flex-1 py-2 px-4 text-sm rounded-md font-semibold transition-colors text-white ${mode === 'BOUNDARY' ? 'bg-amber-500 hover:bg-amber-600' : 'bg-primary hover:bg-sky-500'}`}>{mode === 'BOUNDARY' ? 'Finish Drawing' : 'Draw Boundary'}</button>
                    <button onClick={() => dispatch({ type: 'CLEAR_BOUNDARY' })} disabled={!currentMission.boundary || currentMission.boundary.length === 0} className="flex-1 py-2 px-4 text-sm rounded-md font-semibold transition-colors bg-base-300 hover:bg-slate-600 text-slate-200 disabled:bg-base-300/50 disabled:cursor-not-allowed">Clear Boundary</button>
                </div>
            </div>

            {currentMission.waypoints.length > 1 && <TerrainProfileChart waypoints={currentMission.waypoints} terrainProfile={currentMission.terrainProfile || []} />}
            
            <div className="flex items-center justify-between pt-2"><h4 className="font-semibold text-slate-200">Waypoints ({currentMission.waypoints.length})</h4><button onClick={isSimulating ? stopSimulation : startSimulation} disabled={currentMission.waypoints.length < 2} className={`px-3 py-1 text-sm font-semibold rounded-md transition-colors text-white ${isSimulating ? 'bg-red-500 hover:bg-red-600' : 'bg-green-500 hover:bg-green-600 disabled:bg-base-300 disabled:cursor-not-allowed'}`}>{isSimulating ? 'Stop Simulation' : 'Start Simulation'}</button></div>

            <div className="space-y-2">
                {currentMission.waypoints.length > 0 ? (currentMission.waypoints.map((wp, index) => {
                    ...
                    return (
                        <div key={wp.id} className={`bg-base-100 rounded-md transition-all duration-200 ${isSelected ? 'ring-2 ring-primary' : ''}`}>
                            <div className="flex items-center justify-between p-3 cursor-pointer" onClick={() => setSelectedWaypointId(isSelected ? null : wp.id)}>
                                ...
                                <div className="flex items-center space-x-2"><button onClick={(e) => { e.stopPropagation(); handleDelete(wp.id, index); }} className="text-slate-400 hover:text-accent p-1 rounded-full transition-colors"><TrashIcon /></button><div className={`${isSelected ? 'rotate-180' : ''} transition-transform`}><ChevronDown /></div></div>
                            </div>
                            {isSelected && (
                                <div className="p-3 border-t border-base-300 space-y-3">
                                    <div className="grid grid-cols-2 gap-2 text-sm">
                                        <input type="number" value={wp.lat.toFixed(6)} onChange={e => handleUpdate(wp.id, 'lat', e.target.value)} title="Latitude" className="w-full bg-base-300 p-1 rounded-md focus:ring-1 focus:ring-primary outline-none" />
                                        <input type="number" value={wp.lng.toFixed(6)} onChange={e => handleUpdate(wp.id, 'lng', e.target.value)} title="Longitude" className="w-full bg-base-300 p-1 rounded-md focus:ring-1 focus:ring-primary outline-none" />
                                    </div>
                                    <div className="flex items-center justify-center space-x-2 bg-base-300 p-1 rounded-md">
                                        <span className="text-xs font-medium text-slate-400">Alt Type:</span>
                ...
                })) : (<div className="text-center py-8 bg-base-100 rounded-md"><p className="text-slate-400">Click on the map to add waypoints.</p></div>)}
            </div>
        </div>
    );
};
